...
class NickModal(discord.ui.Modal, title="Finalizar Pedido"):
    nick = discord.ui.TextInput(label="Seu Nick no jogo (opcional)", placeholder="Ex: mrvoid157", required=False)
    ...
    async def on_submit(self, interaction: discord.Interaction):
        # RESPOSTA IMEDIATA: Evita que o painel da loja suma ou dê erro de tempo
        await interaction.response.defer(ephemeral=True)
        cog = interaction.client.get_cog("Shop")
        if cog:
            await cog.gerar_canal_compra(interaction, self.pid, self.nick.value or "Não informado")
...
class Shop(commands.Cog):
    ...
    async def on_interaction(self, interaction: discord.Interaction):
        ...
        if cid.startswith("shop:"):
            # Abre o modal sem "responder" a mensagem original (mantém o painel)
    ...
    async def loja_painel(self, interaction: discord.Interaction):
        ...
        embed = discord.Embed(
            ...
            description="Escolha um produto abaixo para iniciar seu atendimento.",
            color=0x000000
        )
    ...
    async def gerar_canal_compra(self, interaction: discord.Interaction, pid: str, nick: str):
        ...
        embed.add_field(name="💬 O que fazer?", value="Aguarde um staff te atender. Diga o que deseja e peça o PIX.", inline=False)
